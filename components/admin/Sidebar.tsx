...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 shrink-0 bg-darker border-r border-white/5 flex flex-col h-screen sticky top-0">
      <div className="p-5 border-b border-white/5">
        <Image src="/logo.png" alt="Gharmai Drinks" width={120} height={50} className="h-12 w-auto" />
        <p className="text-white/30 text-xs mt-2">Admin CMS</p>
      </div>
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {items.map((n) => {
          const active = path === n.href;
          return (
            <Link key={n.href} href={n.href} className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all ${active ? "bg-gold/10 text-gold border border-gold/20" : "text-white/50 hover:text-white hover:bg-white/5"}`}>
      ...
      <div className="p-4 border-t border-white/5">
        ...
        <div className="text-white/30 text-xs mb-3">{role.replace("_", " ")}</div>
        <button onClick={() => signOut({ callbackUrl: "/admin/login" })} className="w-full py-2 rounded-lg bg-white/5 hover:bg-accent/20 hover:text-accent text-sm transition-all">Sign Out</button>
      </div>
    </aside>
  );
}
