...
export default function CustomersAdmin() {
  ...
  return (
    <div className="p-6 sm:p-8">
      ...
      <p className="text-white/40 text-sm mb-6">{customers.length} customers</p>
      <div className="glass rounded-2xl overflow-x-auto">
        <table className="w-full text-sm min-w-[600px]">
          <thead className="bg-white/5 text-white/50 text-left"><tr><th className="p-3">Name</th><th className="p-3">Phone</th><th className="p-3">VIP Tier</th><th className="p-3">Points</th><th className="p-3">Orders</th><th className="p-3"></th></tr></thead>
          <tbody>
            {customers.map((c) => (
              <tr key={c.id} className="border-t border-white/5">
                ...
                <td className="p-3 text-white/60">{c.rewardPoints}</td>
                <td className="p-3 text-white/50">{c._count?.orders ?? 0}</td>
                <td className="p-3 text-right"><button onClick={() => setEdit({ ...c })} className="text-gold hover:underline">Manage</button></td>
              </tr>
            ))}
            {customers.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-white/30">No customers yet.</td></tr>}
          </tbody>
        </table>
      </div>
      {edit && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-dark-card border border-white/10 rounded-2xl max-w-sm w-full p-6">
            ...
            <label className="text-white/50 text-sm block mb-1">VIP Tier</label>
            ...
            <label className="text-white/50 text-sm block mb-1">Reward Points</label>
            <input type="number" value={edit.rewardPoints} onChange={(e) => setEdit({ ...edit, rewardPoints: e.target.value })} className="inp mb-4" />
            <div className="flex gap-3"><button onClick={save} className="flex-1 py-3 btn-gold rounded-xl">Save</button><button onClick={() => setEdit(null)} className="px-6 py-3 rounded-xl bg-white/5">Cancel</button></div>
          </div>
        </div>
      )}
