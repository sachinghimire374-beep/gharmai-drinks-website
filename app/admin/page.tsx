...
export default function Dashboard() {
  ...
  if (err) return <div className="p-8"><p className="text-white/50">{err === "Forbidden" ? "Your role does not have analytics access." : err}</p></div>;
  if (!s) return <div className="p-8 text-white/40">Loading dashboard…</div>;
  ...
  return (
    <div className="p-6 sm:p-8">
      ...
      <p className="text-white/40 text-sm mb-6">Overview of your store performance</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        {cards.map(([label, val, icon]) => (
          <div key={label as string} className="glass rounded-2xl p-5">
            ...
            <div className="text-white/40 text-xs mt-1">{label as string}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="glass rounded-2xl p-6">
          ...
          {s.topItems.length === 0 && <p className="text-white/30 text-sm">No sales yet.</p>}
          {s.topItems.map((t: any, i: number) => (
            <div key={i} className="flex justify-between py-2 border-b border-white/5 last:border-0">
              <span className="text-white/60 text-sm">{i + 1}. {t.name}</span>
        ...
        <div className="glass rounded-2xl p-6">
          ...
          {s.adStats.length === 0 && <p className="text-white/30 text-sm">No ads tracked yet.</p>}
          {s.adStats.map((a: any) => (
            <div key={a.id} className="flex justify-between py-2 border-b border-white/5 last:border-0">
              <span className="text-white/60 text-sm truncate mr-2">{a.title}</span>
              <span className="text-white/40 text-xs whitespace-nowrap">{a.clicks}/{a.impressions} · <span className="text-gold">{a.ctr}%</span></span>
            </div>
          ))}
        </div>
      </div>
